...
TEST_CASE("GeneratePrimeNumbersSet с граничными значениями")
{
    SECTION("обрабатывает 0")
    {
    ...
    SECTION("upperBound равен 3")
    {
        auto result = GeneratePrimeNumbersSet(3);
        std::set<uint64_t> expected = {2, 3};
        REQUIRE(result == expected);
    }

    SECTION("upperBound само является простым числом")
    {
        auto result = GeneratePrimeNumbersSet(29);
        REQUIRE(result.find(29) != result.end());
    }
}

TEST_CASE("GeneratePrimeNumbersSet обрабатывает переполнение")
{
    SECTION("отрицательное число")
    {
        auto negative = static_cast<uint64_t>(-5);
        CHECK_THROWS_AS(GeneratePrimeNumbersSet(negative), std::overflow_error);
    }

    SECTION("максимальное отрицательное число")
    {
        auto negative_one = static_cast<uint64_t>(-1);
        CHECK_THROWS_AS(GeneratePrimeNumbersSet(negative_one), std::overflow_error);
    }

    SECTION("upperBound равен INT_MAX бросает исключение")
    {
        CHECK_THROWS_AS(GeneratePrimeNumbersSet(INT_MAX), std::overflow_error);
    }
}

TEST_CASE("GeneratePrimeNumbersSet корректность результата")
{
    SECTION("находит первые простые числа до 30")
    {
    ...
    SECTION("даёт 25 простых чисел до 100")
    {
        auto result = GeneratePrimeNumbersSet(100);
        REQUIRE(result.size() == 25);
    }

    SECTION("проверяем конкретные числа до 50")
    {
